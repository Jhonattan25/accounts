pipeline {
    ...
    environment {
        def imageName = "jhonattan25/app-microservicios-accounts"
    ...
    stages {
        stage('Build Maven') {
            steps {
                checkout scmGit(branches: [[name: '*/master']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/Jhonattan25/accounts.git']])
        ...
        stage('Deploy to K8s') {
            ...
            steps{
                ...
                sh 'ls -ltr'
                sh 'pwd'
                sh "sed -i 's/tagversion/${tag}/g' deployment/accounts.yaml"
                step([$class: 'KubernetesEngineBuilder', \
                  projectId: env.PROJECT_ID, \
                  clusterName: env.CLUSTER_NAME, \
                  location: env.LOCATION, \
                  manifestPattern: 'deployment/accounts.yaml', \
